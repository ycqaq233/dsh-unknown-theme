...
A static client bundle that restyles the DeepSeek Harness web UI with the visual language of deepseek.com — the fluid gradient background, the interactive dot-grid, the fish-shaped particle logo, the spotlight ring, and the frosted-glass components lifted from the harness landing page.
