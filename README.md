...
| 🐳 **粒子鱼形 LOGO** | 新会话页粒子鱼；入场组合动画期间鼠标不产生斥力，鱼形完整成形后才可被打散；入场与回正统一为**指数收敛**（越靠近中心点恢复越快）——入场快速成形（≈1.2s），成形后被打散以慢系数游回（190px 散开约 22 秒）；单例挂载管理，防双份粒子叠加 |
...
| `LERP_ENTRY` | 粒子入场组合动画指数系数（位移 = 剩余距离 × LERP，越大越快） |
| `LERP_SLOW` | 成形后被打散的慢速指数回正系数（越靠近中心点恢复越快） |
...
- **调粒子手感**：`LERP_ENTRY`（入场）/ `LERP_SLOW`（慢速回正，粒子 tick 内）
