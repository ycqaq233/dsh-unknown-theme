/**
 * dsh-unknown-theme — Client half (static web bundle).
 *
 * DeepSeek 官网视觉复刻（代码级）：
 *   - 会话主区：官网原版 WebGL 流体 shader（snoise+fbm+域扭曲+curl+5色，highp，
 *     对比拉伸 + dither 防色带），canvas 加 blur(9px) 毛玻璃柔化条纹
 *   - 玻璃组件（输入卡/用户气泡/新会话按钮/日志按钮）：harness 官网终端卡片同款
 *     ——bg-black/20 + backdrop-blur-xl(24px) + 1px 白 8% 描边（不搬圆点装饰）
...
`
    ...
    function apply(ctx) {
      ...
      styleEl.dataset.plugin = 'dsh-unknown-theme'
      styleEl.textContent = `
        ...
        /* 官网质感玻璃（harness hero 终端卡片同款）：
           bg-black/20 深色 tint + backdrop-blur-xl(24px) 磨砂 + 1px 白 8% 描边。
           不搬左上角三个圆点，只搬质感：无折射层、无高光、无外阴影 */
        [data-composer-card],
        [class$="_userStack"] [class$="_bubble"],
        .hHd-Xa_newSession,
        .nL4_yW_sessionLogButton {
          position: relative !important;
          background: var(--lg-bg, rgba(0,0,0,.2)) !important;
          border: 1px solid var(--lg-border, rgba(255,255,255,.08)) !important;
        ...
        .nL4_yW_sessionLogButton:hover:not(:disabled) {
          background: var(--lg-bg-hover, rgba(0,0,0,.3)) !important;
        }
      ...
      // ---------- 2.5 液态玻璃 SVG filter（已弃用：CSS 已改官网磨砂质感，定义保留备用）
      ...
      const paintTheme = () => {
        ...
        if (dark) {
          ...
          root.style.setProperty('--lg-bg', 'rgba(0,0,0,.2)')
          root.style.setProperty('--lg-border', 'rgba(255,255,255,.08)')
          root.style.setProperty('--lg-bg-hover', 'rgba(0,0,0,.3)')
          ...
          document.body.style.setProperty('--dsw-specific-sidebar-fill', 'rgba(0,0,0,0)')
          root.style.setProperty('--lg-bg', 'rgba(255,255,255,.3)')
          root.style.setProperty('--lg-border', 'rgba(255,255,255,.7)')
          root.style.setProperty('--lg-bg-hover', 'rgba(255,255,255,.45)')
      ...
      cleanups.push(() => {
        const root = document.documentElement
        for (const v of ['--ds-ds-bg', '--ds-ds-bg-end', '--ds-sidebar-grad', '--lg-bg', '--lg-border', '--lg-bg-hover', '--ds-particle-fill', '--ds-fluid-1', '--ds-fluid-2', '--ds-fluid-3', '--ds-fluid-4', '--ds-fluid-5', '--ds-grid-line', '--ds-grid-dot', '--ds-grid-dot-a']) {
      ...
      const onPointerMove = (e) => {
        ...
        if (t instanceof Element) {
          // 只在标题文字或鱼图标上触发聚光灯，容器空白区（无文字/图标）不触发
          on = !!t.closest('[class$="_headlineText"], [class$="_fishHitbox"]')
        }
      ...
      const initFluid = () => {
        ...
        const compile = (type, src) => {
          ...
          if (!fluidGl.getShaderParameter(sh, fluidGl.COMPILE_STATUS)) {
            console.error('dsh-unknown-theme: fluid shader error', fluidGl.getShaderInfoLog(sh))
            return null
          }
          return sh
        }
      ...
      // ---------- 7. 粒子 LOGO（FishLogo 形状采样 + 动量斥力 + 指数回正） ----------
      let canvas = null
      let host = null
      let phaseHero = null
      let raf = 0
      ...
      const layout = () => {
        ...
        entered = false   // 重采样 = 重播入场动画（用快速回正）
      ...
      const tick = () => {
        ...
        // 4. 斥力 + 回正（惯性滑行在鼠标离开后快速刹停）
        const ctx2 = canvas.getContext('2d')
        const hb = host.getBoundingClientRect()
        const mx = mouse.x - hb.left
        const my = mouse.y - hb.top
        const R = 19
        const LERP_ENTRY = 0.08      // 入场组合动画指数系数（快速成形：位移 = 剩余距离 × LERP，
                                     //   190px 散开 ≈ 1.2s 组合完成）
        const LERP_SLOW = 0.0045     // 成形后被打乱的慢速指数回正（≈ 22s，比旧版 0.012 慢约 2.7 倍）
        const DAMP_HOLD = 0.86       // 扫过期间：慢阻尼，累积冲力把粒子打散
        const DAMP_BRAKE = 0.5       // 鼠标离开后：快阻尼，惯性滑行 ~0.1s 内刹停
        const VMAX = 24              // 速度上限，防止打飞太远
        // 回正统一为指数收敛：入场（entered=false）用快系数快速成形；
        // 成形后（entered=true）用慢系数——越靠近中心点恢复越快，整体保持慢速
        const lerp = entered ? LERP_SLOW : LERP_ENTRY
        let anyMoving = false
        for (const p of particles) {
          ...
          // 斥力：仅在成形后（entered）生效；入场动画期间鼠标不干扰，保证动画顺利完成
          if (entered && d2 < R * R && d2 > 0.01) {
            const dist = Math.sqrt(d2)
            const f = (1 - dist / R) * (1 - dist / R) * 30
            p.vx += (dx / dist) * f
            p.vy += (dy / dist) * f
            lastRepelFrame = frameCount
            anyMoving = true
          }
          // 速度上限（防打飞太远）
          const sp = Math.sqrt(p.vx * p.vx + p.vy * p.vy)
          if (sp > VMAX) { p.vx *= VMAX / sp; p.vy *= VMAX / sp }
          // 惯性刹停：鼠标离开 3 帧后动量快速衰减
          const damp = (frameCount - lastRepelFrame) > 3 ? DAMP_BRAKE : DAMP_HOLD
          p.vx *= damp
          p.vy *= damp
          p.x += p.vx
          p.y += p.vy
        ...
        settled = !anyMoving
        if (settled) entered = true   // 首次成形完成 → 之后被打乱用慢速回正
        dirty = false
      }
