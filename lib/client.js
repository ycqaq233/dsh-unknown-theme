...
`
    ...
    function apply(ctx) {
      ...
      const tick = () => {
        ...
        // 4. 斥力 + 匀速回正（固定步长；惯性滑行在鼠标离开后快速刹停）
        const ctx2 = canvas.getContext('2d')
        const hb = host.getBoundingClientRect()
        const mx = mouse.x - hb.left
        const my = mouse.y - hb.top
        const R = 19
        const RETURN_STEP_ENTRY = 3    // 入场组合动画速度 px/帧（60fps ≈ 180px/s）
        const RETURN_STEP_SLOW = 0.15  // 成形后被打乱的慢速回正 px/帧（≈ 9px/s，散开约 20s 游回）
        const DAMP_HOLD = 0.86         // 扫过期间：慢阻尼，累积冲力把粒子打散
        const DAMP_BRAKE = 0.5         // 鼠标离开后：快阻尼，惯性滑行 ~0.1s 内刹停
        const VMAX = 24                // 速度上限，防止打飞太远
        // 两阶段：首次成形前用快速（入场动画）；成形后被打乱用慢速回正
        const step = entered ? RETURN_STEP_SLOW : RETURN_STEP_ENTRY
        let anyMoving = false
